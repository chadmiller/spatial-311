...
def api_get_point_info(request):
    ...
    with psycopg2.connect("dbname='{}'".format(db_name)) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT topic.name, parcel_tags.id, parcel_tags.key, parcel_tags.value, parcel_tags.make_visible FROM parcel_tags LEFT JOIN parcel on (parcel_tags.parcel=parcel.id) LEFT JOIN import ON (parcel.import=import.id) LEFT JOIN topic on (topic.id=import.topic) WHERE ST_Contains(parcel.shape, ST_MakePoint({},{})) ORDER BY topic.name, parcel_tags.ordering, parcel_tags.key;".format(lng, lat))
